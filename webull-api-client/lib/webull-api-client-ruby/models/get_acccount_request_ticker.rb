...
module WebullApiClient
  class GetAcccountRequestTicker
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WebullApiClient::GetAcccountRequestTicker` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WebullApiClient::GetAcccountRequestTicker`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [currency_code, currency_id, dis_exchange_code, dis_symbol, exchange_code, exchange_id, ext_type, list_status, name, region_id, region_iso_code, region_name, sec_type, symbol, ticker_id, tiny_name, type].hash
    end
